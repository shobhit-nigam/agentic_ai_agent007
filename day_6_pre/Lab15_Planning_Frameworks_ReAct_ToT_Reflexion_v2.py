...
# Lab 15: Planning Frameworks — ReAct, Tree-of-Thoughts (ToT), Reflexion (v2, with hardening)
# ===========================================================================================
# Changes vs v1:
# 1) Input normalization: before executing `suggest_city_activities`, we auto-fill
#    `city` and `weather` from context so the tool gets canonical input.
# 2) Critic-on-override: when Validator overrides a `done:true` (policy miss), in
#    Reflexion mode we route to CRITIC to generate a reflection before continuing.
#
# Prereqs
...
#
# Run
#   python Lab15_Planning_Frameworks_ReAct_ToT_Reflexion_v3.py
...
class AgentState(TypedDict):
    ...
    reflection_hint: Optional[str]      # Validator-provided hint when overriding
...
def normalize_step(step: dict, messages: List[AnyMessage]) -> dict:
    """Ensure `suggest_city_activities` gets canonical 'city=...; weather=...' when possible."""
    if not step:
        return step
    tool = step.get("tool")
    ...
    # Try to infer city + weather from context
    user = latest_user_text(messages)
    city = city_from_text(user) or ""
    # If weather tool already ran, extract its text
    weather = extract_weather_text(messages, city) if city else None
    weather = weather or ""
    # If the input already includes city=..., keep it; else fill it
    if "city=" not in inp.lower():
        if city:
            if weather:
                inp = f"city={city}; weather={weather}"
            else:
                inp = city
        # else leave as-is (planner might have passed a valid city string)
...
def validator_node(state: AgentState) -> AgentState:
    """Enforce simple policies before allowing done:true.
       If overriding a final in Reflexion mode, attach a reflection_hint so Critic can teach the Planner.
    """
    ...
    if plan.get("done") is True:
        ...
        if city:
            if not has_tool_result(state["messages"], "get_weather"):
                return {
                    ...
                    "reflection_hint": "I attempted to suggest activities without checking weather first. Next time, call get_weather(city) before activities."
                }
            if not has_tool_result(state["messages"], "suggest_city_activities"):
                ...
                return {
                    ...
                    "reflection_hint": "I finalized before suggesting activities after weather. Next time, include suggest_city_activities after get_weather."
                }
        # Math policy
        if need_math and not has_tool_result(state["messages"], "calculator"):
            return {
                ...
                "reflection_hint": "I finalized without computing the requested math. Next time, call calculator(expression) before finalizing."
            }
        # All good → allow END
        return {}
    # Not done → continue
...
def executor_node(state: AgentState) -> AgentState:
    """Execute exactly one tool step or summarize a tool result.
       Tools are disabled during summarization to avoid accidental calls.
       Before executing suggest_city_activities, normalize inputs.
    """
    ...
    step = normalize_step(step, msgs)  # <-- normalization
...
def critic_node(state: AgentState) -> AgentState:
    ...
    hint = (state.get("reflection_hint") or "").strip()
    if hint:
        reflection = hint
    else:
        prompt = [
    ...
    return {"messages": [AIMessage(content=f"[CRITIC] {reflection}")], "reflection": reflection, "attempt": state.get("attempt", 0) + 1, "reflection_hint": None}
...
def route_from_validator(state: AgentState):
    ...
    if plan.get("done") is True:
        return END
    # If validator suggests a next_step → in Reflexion mode, go to CRITIC first if we just overrode a final
    if plan.get("next_step"):
        if state.get("reasoning_mode") == "reflexion" and state.get("reflection_hint"):
            return "critic"
        return "executor"
...
def run_demo_react():
    cfg = {"configurable": {"thread_id": "lab15-react-v3"}, "recursion_limit": 40}
    state: AgentState = {"messages": [HumanMessage(content="Plan a short evening in Chicago. First tell weather, then one indoor and one outdoor pick.")],
                         "plan": None, "candidates": None, "reflection": None, "reflection_hint": None, "attempt": 0, "turn": 0, "reasoning_mode": "react"}
