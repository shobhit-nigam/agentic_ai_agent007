...
def call_with_policies(tool_name: str, tool_fn: Callable[..., str], args: Dict,
                       *, denylist, max_attempts=2, backoff_sec=0.3, enable_logging=True) -> str:
    ...
    while attempts < max_attempts:
        try:
            ...
            if enable_logging: log.info(f"OK   {tool_name} in {dt}ms out={str(out)[:200]}")
            return out
            ...
            if attempts >= max_attempts:
                ...
                return f"Sorry — the {tool_name} tool failed after retries: {last_err}"
