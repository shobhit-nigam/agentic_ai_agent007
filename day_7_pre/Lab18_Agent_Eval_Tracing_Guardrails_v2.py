# Lab 18 v2 (CLEAN) — Agent Evaluation, Tracing & Safety Guardrails
# Fully self-contained; avoids passing ToolMessage to ChatOpenAI and strips assistant tool_calls from history.
...
class WeatherInput(BaseModel):
    """Inputs for live/cache weather tools"""
    city: str = Field(..., description="City name, e.g., 'Paris'")
    units: Literal["metric","imperial"] = Field("metric")

class WeatherOutput(BaseModel):
    """Unified weather output schema used by both live and cache tools"""
    city: str
    temp_c: float
...
class CalcInput(BaseModel):
    """Arithmetic expression to evaluate safely"""
    expression: str
...
def normalization_for_activities(messages: List[AnyMessage]) -> Optional[str]:
    ...
    if tm and getattr(tm, "name", "") in ["weather_live", "weather_cache"]:
        ...
        if st["status"] == "OK" and st["kind"] == "WEATHER":
            ...
            temp = d.get("temp_c","")
            weather_str = f"{cond}, {temp}°C".strip(", ")
    if weather_str:
        return f"city={city}; weather={weather_str}"
    return None

def _llm_safe_messages(msgs: List[AnyMessage]) -> List[AnyMessage]:
    """Convert ToolMessage -> AIMessage; keep Human/AI/System as-is."""
    ...
    for m in msgs:
        if isinstance(m, ToolMessage):
            ...
            safe.append(AIMessage(content=f"[TOOL:{getattr(m,'name','?')}] {content}"))
        else:
            safe.append(m)
    return safe

def _openai_legal_context(msgs: List[AnyMessage]) -> List[AnyMessage]:
    """Drop assistant messages that contain tool_calls; convert ToolMessage to AIMessage text."""
    cleaned: List[AnyMessage] = []
    for m in msgs:
        if isinstance(m, AIMessage):
            tc = getattr(m, "tool_calls", None) or (getattr(m, "additional_kwargs", {}) or {}).get("tool_calls")
            if tc:
                # Skip assistant messages that issued tool_calls
                continue
            cleaned.append(m)
            ...
            if len(content) > 1000:
                content = content[:1000] + "…"
            cleaned.append(AIMessage(content=f"[TOOL:{getattr(m,'name','?')}] {content}"))
        else:
            cleaned.append(m)
    return cleaned
...
PLANNER_SYS = (
    ...
    "Policy (MANDATORY): If the user mentions a city, the FIRST step MUST be weather_live.\n"
    "If and only if weather_live fails (or the breaker is open), you MAY use weather_cache.\n"
    "NEVER call suggest_city_activities until a weather ToolMessage (weather_live or weather_cache) exists in the conversation history.\n"
...
# ---------- Nodes ----------
def planner_node(state: AgentState, tracer: Tracer, planner_llm) -> AgentState:
    tracer.log("planner", "before", {"messages_len": len(state["messages"])})
    resp = planner_llm.invoke([SystemMessage(content=PLANNER_SYS)] + _openai_legal_context(state["messages"]))
...
def runtime_validator_node(state: AgentState, tracer: Tracer) -> AgentState:
    plan = state.get("plan") or {}
    msgs = state["messages"]
    user = latest_user_text(msgs).lower()
    tracer.log("validator", "before", {"plan": plan})

    # Helper: have we already seen a weather result?
    ...
    # EARLY ENFORCEMENT: If planner tries activities before weather, override to weather_live
    ns = plan.get("next_step") or {}
    if ns.get("tool") == "suggest_city_activities" and not (seen("weather_live") or seen("weather_cache")):
        city = infer_city_from_user(msgs)
        new_plan = {"done": False, "next_step": {"tool":"weather_live","input": json.dumps({"city": city})}, "rationale":"Policy: weather must precede activities."}
        tracer.log("validator","after",{"action":"force_weather_before_activities","new_plan":new_plan})
        return {"plan": new_plan, "violations": state["violations"]+1}

    # If previous tool was a failing weather_live → fallback to cache
    ...
    # If planner tries to finalize, ensure policies satisfied
    if plan.get("done") is True:
        needs_weather = any(c in user for c in ["paris","chicago","london"])

        def seen_any_weather(): return seen("weather_live") or seen("weather_cache")

        if needs_weather and not seen_any_weather():
            city = infer_city_from_user(msgs)
        ...
        if re.search(r"\d+\s*[\+\-\*/]\s*\d+", user) and not seen("calculator_strict"):
            expr_m = re.search(r"([0-9+\-*/(). %]+)", user)
            expr = expr_m.group(1) if expr_m else "0"
    ...
    # If planner proposed a next_step and it isn't illegal, pass through
    if plan.get("next_step"):
        tracer.log("validator", "after", {"action":"pass_next_step"})
        return {}

    # Default: weather first
    city = infer_city_from_user(msgs)
...
def executor_node(state: AgentState, tracer: Tracer, executor_llm_with_tools) -> AgentState:
    msgs = list(state["messages"])
    last = msgs[-1]
    # If last is ToolMessage → summarize
    if isinstance(last, ToolMessage):
        ...
        resp = ChatOpenAI(model=DEFAULT_MODEL, temperature=0).invoke(
            _openai_legal_context(msgs) + [SystemMessage(content="Return one line 'EXEC_RESULT: ...'"),
                    HumanMessage(content="EXEC_RESULT: summarize last tool result")]
        )
        return {"messages":[resp]}

    # Otherwise, execute exactly one step
    step = (state.get("plan") or {}).get("next_step") or {}
    tool_name = step.get("tool","")
    raw_input = step.get("input","")

    # Normalize inputs as needed
    ...
    resp = executor_llm_with_tools.invoke(
        _openai_legal_context(msgs) + [SystemMessage(content=EXECUTOR_SYS),
                HumanMessage(content=f"Execute ONLY this step: {json.dumps({'tool': tool_name, 'input': parsed})}")]
    )
    # If the model emitted a tool call, proceed as usual
    tcs = getattr(resp, "tool_calls", None) or (getattr(resp, "additional_kwargs", {}) or {}).get("tool_calls")
    if tcs:
        tracer.log("executor", "after", {"tool": tool_name, "raw": getattr(resp, "content", "")[:200]})
        return {"messages":[resp], "tool_calls": state["tool_calls"]+1}

    # Otherwise, programmatically call the tool to avoid loops
    tool_obj = tool_map.get(tool_name)
    if tool_obj is not None:
        try:
            content = tool_obj.invoke(parsed)
        except Exception as e:
            content = f"ERROR|{tool_name.upper()}|{{\"code\":\"INVOKE\",\"message\":\"{str(e)}\"}}"
        tm = ToolMessage(content=content, name=tool_name, tool_call_id=f"manual-{uuid.uuid4().hex}")
        tracer.log("executor", "after", {"tool": tool_name, "raw": str(content)[:200], "mode":"manual_invoke"})
        return {"messages":[tm], "tool_calls": state["tool_calls"]+1}
    else:
        tracer.log("executor", "after", {"tool": tool_name, "raw": getattr(resp, "content", "")[:200], "mode":"no_tool_found"})
        return {"messages":[resp]}
...
def build_app(tracer: Tracer):
    ...
    def planner_wrap(state: AgentState) -> AgentState:
        return planner_node(state, tracer, planner_llm)
    def validator_wrap(state: AgentState) -> AgentState:
        return runtime_validator_node(state, tracer)
    def executor_wrap(state: AgentState) -> AgentState:
        return executor_node(state, tracer, executor_llm_with_tools)
...
def run_one_case(case: EvalCase, tracer: Tracer, thread_id: str) -> Dict[str, Any]:
    # Reset simulation flags and breaker
...
def score_case(case: EvalCase, result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def check(name, cond, msg):
        checks.append({"name": name, "pass": bool(cond), "msg": msg})
    if "weather_first" in exp:
        check("weather_first", ana["weather_first"] == exp["weather_first"], f"Expected weather_first={exp['weather_first']}, got {ana['weather_first']}")
    if "activities_present" in exp:
        check("activities_present", ana["activities_present"] == exp["activities_present"], f"Expected activities_present={exp['activities_present']}, got {ana['activities_present']}")
    if exp.get("used_cache"):
        check("used_cache", ana["used_cache"] is True, "Expected use of cache after live failure")
    if "violations_max" in exp:
        check("violations_max", result["violations"] <= exp["violations_max"], f"violations={result['violations']} <= {exp['violations_max']}")
    if "violations_min" in exp:
        check("violations_min", result["violations"] >= exp["violations_min"], f"violations={result['violations']} >= {exp['violations_min']}")
    if "requires_math" in exp:
        check("math_present", ana["math_present"] is True, "Expected calculator_strict to be used")
        if exp.get("numeric_only"):
            check("numeric_only_final", ana["numeric_only_final"] is True, "Expected numeric-only final")
...
def main():
    print("\n=== Lab 18 v2 — Agent Evaluation, Tracing & Safety Guardrails (CLEAN) ===")
    cases = default_dataset()
    report = run_eval_suite(cases, run_name="lab18v2")
